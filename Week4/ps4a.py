...
def getWordScore(word, n):
    ...
    ## initalize score to zero for the word
    score = 0

    ## loop through each letter of word and get the score for the letter
    for i in range (0, len(word)):
        score += SCRABBLE_LETTER_VALUES[word[i]]

    ## get the final score before bonus with teh len(word)
    score *= len(word)


    ## if all words in the hand are used, then give additional 50 points
    if len(word) == n:
        score += 50
...
def updateHand(hand, word):
    ...
    ## creat copy fo the hand dictionary
    handCopy = hand.copy()

    ## iterate through each letter of word
    for i in range(len(word)):
        ## if that letter is in the copy of hand, reduce value in dictionary by 1
        if word[i] in handCopy:
            handCopy[word[i]] -= 1

    ## return the copy fo the hand
    return handCopy
...
def calculateHandlen(hand):
    ...
    returns: integer
    """
    ## counting variable for tracking total length
    lenHand = 0
    ## iterate through each key in the dictionary, adding the value for each to counting variable
    for letter in hand:
        lenHand += hand[letter]
    return lenHand
...
def playGame(wordList):
    ...
    """
    # creating a blank dictionary to hold the hand
    hand = {}

    # continuous loop for game play
    while True:
        play = input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
    

        # new hand; deal a new hand and initiate playHand swquence
        if play == "n":
            hand = dealHand(HAND_SIZE)
            playHand(hand, wordList, HAND_SIZE)

        # but if you want to replay the previous hand
        elif play == "r":
            # error message if you have not played a previous hand
            if hand == {}:
                print("You have not played a hand yet. Please play a new hand first!")
            # but if there is a previous hand, initiate the playHand function
            elif hand != {}:
                playHand(hand, wordList, HAND_SIZE)
            
        # break out of the continous loop if desired and end the game
        elif play == "e":
            break

        # error message if uncooperative user gives bad input
